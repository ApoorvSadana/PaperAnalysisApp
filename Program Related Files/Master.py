...
answer_coordinates = []
def everything(path_to_color_image,path_to_unprepared_images,path_to_prepared_images,path_to_answer_sheets,sheet_number):
    
    colored_final_img = cv2.imread(path_to_color_image)
    black_linear_img = cv2.imread(path_to_color_image,0)
    ...
    #scanned copy
    ret,thresh = cv2.threshold(black_linear_img,127,255,cv2.THRESH_BINARY_INV)
    ...
    if len(contours)>10:
        ...
        for t in range(len(contours)):
            ...
            if h>max_height and x+w<black_linear_img.shape[1]/3:
            ###########################################################################
                ci = t
                max_height = h
    else:
    ...
    crop_black_img = dilated[y:y+h, x:x+w]    #For Table
    ...
    im2, contours, hierarchy = cv2.findContours(final_black,cv2.RETR_TREE,cv2.CHAIN_APPROX_SIMPLE)
...
def main(path_to_color):
    ...
    files_color.sort()
    for a in range(len(files_color)):
        sheet_number = a+1
        ...
        record = everything(path_to_color + files_color[a],path_to_unprepared_images,path_to_prepared_images,path_to_answer_sheets,sheet_number)
        spreadsheet.append(record)
    return spreadsheet

#path_to_linear = 'AnsLinear7/'
path_to_color = 'AnsColor9/'
spreadsheet = main(path_to_color)
